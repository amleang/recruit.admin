<template>
    <div>
        ...
        <base-table tableName="待入职列表" ref="table" :action="table.action" :columns="table.heads" :queryParams="queryParams">
            <template slot-scope="props">
                <div v-if="props.tplName=='toperation'">
                    ...
                    <el-button size="small" icon="el-icon-edit" type="primary" @click="editor_handle(props.row,3)">待返现
                    </el-button>
                    <el-button size="small" icon="el-icon-success" type="danger" @click="editor_handle(props.row,4)">完结</el-button>
                </div>
            </template>
        </base-table>
...
export default {
    ...
    return {
      ...
      table: {
        ...
        heads: [
          ...
          { prop: "entryAt", label: "入职时间", type: "time" },
          {
            prop: "operation",
            label: "操作",
            type: "comps",
  ...
  methods: {
    ...
    editor_handle(row, status) {
      let msg = "确定要设置该信息为待返现吗?";
      if (status == 4) msg = "确定要设置该信息为已完结吗?";
      this.$confirm(msg, "提示", {
